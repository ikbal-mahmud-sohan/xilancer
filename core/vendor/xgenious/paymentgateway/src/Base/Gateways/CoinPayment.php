...
class CoinPayment extends PaymentGatewayBase
{
    ...
    public function ipn_response(array $args = [])
    {
        ...
        if (!Request::header('HTTP_HMAC') || empty(Request::header('HTTP_HMAC'))) {
        ...
        if ($request === FALSE || empty($request)) {
            abort(400, "Error reading POST data");
        }

        $hmac = Hash::hmac('sha512', $request, $ipn_secret);
        if ($hmac != Request::header('HTTP_HMAC')) {
        ...
        if(strtoupper($status_text) === 'complete' && $status === 100){
